...
<div class="card">
    ...
    <form action="{{ route('vendas.update', $venda->id) }}" method="POST" id="form-venda">
        ...
        <div class="card-body">
                ...
                <div class="col-md-6">
                    ...
                    <select name="cliente_id" class="form-control">
                        ...
                        @foreach($clientes as $c)
                            <option value="{{ $c->id }}" {{ old('cliente_id', $venda->cliente_id)==$c->id ? 'selected' : '' }}>
                                {{ $c->nome }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="forma_pagamento" id="forma_pagamento" class="form-control" required>
                        @foreach(['Dinheiro','Cartão','Parcelado','Personalizado'] as $fp)
                            <option value="{{ $fp }}" {{ old('forma_pagamento', $venda->forma_pagamento)==$fp ? 'selected' : '' }}>
                                {{ $fp }}
                            </option>
                        @endforeach
                    </select>
                ...
                <tbody>
                    ...
                    @foreach($oldItens as $i => $item)
                        <tr class="item-row">
                            <td>
                                <select name="itens[{{ $i }}][produto_id]" class="form-control produto-select" required>
                                    ...
                                    @foreach($produtos as $p)
                                        <option value="{{ $p->id }}" {{ $item['produto_id']==$p->id ? 'selected' : '' }}>
                                            {{ $p->nome }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td>
                                <input type="number" name="itens[{{ $i }}][quantidade]" class="form-control quantidade" min="1" value="{{ $item['quantidade'] }}" required>
                            </td>
                            <td>
                                <input type="text" name="itens[{{ $i }}][preco_unitario]" class="form-control preco-unitario" value="{{ number_format($item['preco_unitario'] ?? 0,2,',','.') }}" required>
                            </td>
                            <td>
                                <input type="text" class="form-control total-item" value="{{ number_format(($item['preco_total'] ?? ($item['quantidade'] * ($item['preco_unitario'] ?? 0))),2,',','.') }}" readonly>
                            </td>
                            <td>
                                <button type="button" class="btn btn-sm btn-danger remover-item" aria-label="Remover item">
            ...
            <button type="button" class="btn btn-sm btn-success mb-3" id="adicionar-item" aria-label="Adicionar item">
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6">
                    <input type="text" id="total-venda" name="total" class="form-control text-right" value="{{ number_format($venda->total,2,',','.') }}" readonly>
                </div>
            </div>
            ...
            <div id="parcelamento" style="display:{{ old('forma_pagamento',$venda->forma_pagamento)=='Parcelado' ? 'block' : 'none' }};margin-top:30px">
                ...
                <div class="row mb-2">
                    <div class="col-md-4">
                        ...
                        @php $selParc = old('qtd_parcelas', $venda->parcelas->count()); @endphp
                        <select id="qtd_parcelas" class="form-control">
                            @for($i=2; $i<=12; $i++)
                                <option value="{{ $i }}" {{ $selParc==$i ? 'selected' : '' }}>{{ $i }}x</option>
                            @endfor
                        </select>
                    </div>
                </div>
                <table class="table table-sm" id="tabela-parcelas">
                    ...
                    <tbody>
                        @php $oldParcelas = old('parcelas', $venda->parcelas->toArray()); @endphp
                        @foreach($oldParcelas as $i => $parc)
                            <tr>
                                <td>{{ $i+1 }}<input type="hidden" name="parcelas[{{ $i }}][numero]" value="{{ $i+1 }}"></td>
                                <td>
                                    <input type="date" name="parcelas[{{ $i }}][data_vencimento]" class="form-control" value="{{ old('parcelas.'.$i.'.data_vencimento', $parc['data_vencimento']) }}">
                                </td>
                                <td>
                                    <input type="text" name="parcelas[{{ $i }}][valor]" class="form-control" value="{{ number_format(old('parcelas.'.$i.'.valor', $parc['valor']),2,',','.') }}">
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            <div id="personalizado" style="display:{{ old('forma_pagamento',$venda->forma_pagamento)=='Personalizado' ? 'block' : 'none' }};margin-top:30px">
                ...
                <div class="row mb-2">
                    <div class="col-md-4">
                        ...
                        <select id="qtd_parcelas_personalizado" class="form-control">
                            @for($i=2; $i<=12; $i++)
                                <option value="{{ $i }}" {{ $selP==$i ? 'selected' : '' }}>{{ $i }}x</option>
                            @endfor
                        </select>
                    </div>
                </div>
                <table class="table table-bordered" id="tabela-personalizado">
                    ...
                    <tbody>
                        @php
                            $rows = old('parcelas_personalizadas') ? old('parcelas_personalizadas') : $venda->parcelas->toArray();
                        @endphp
                        @foreach($rows as $i => $p)
                            <tr>
                                <td>{{ $i+1 }}<input type="hidden" name="parcelas_personalizadas[{{ $i }}][numero]" value="{{ $i+1 }}"></td>
                                <td>
                                    <input type="date" name="parcelas_personalizadas[{{ $i }}][data_vencimento]" class="form-control" value="{{ old('parcelas_personalizadas.'.$i.'.data_vencimento', $p['data_vencimento']) }}">
                                </td>
                                <td>
                                    <input type="text" data-idx="{{ $i }}" name="parcelas_personalizadas[{{ $i }}][valor]" class="form-control valor-parcela-personalizada" value="{{ number_format(old('parcelas_personalizadas.'.$i.'.valor', $p['valor']),2,',','.') }}">
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
function atualizarTotais(){
    ...
    $('#tabela-itens tr').each(function(){
        ...
        let pu = parseFloat($(this).find('.preco-unitario').val().replace(/\./g,'').replace(',','.'))||0;
        let ti = q * pu;
...
function gerarParcelas(){
    ...
    let base = Math.floor((total/qtd)*100)/100;
    let resto = Math.round((total - base*qtd)*100)/100;
    let hoje = new Date();
    let html = '';
    for(let i=0; i<qtd; i++){
...
function gerarPersonalizado(reset=false){
    ...
    for(let i=0; i<qtd; i++){
        ...
        let val = reset || existing===undefined
        ...
        let dt = dtEl || new Date(hoje.getFullYear(), hoje.getMonth()+i+1, hoje.getDate()).toISOString().slice(0,10);
        html += `<tr>
            <td>${i+1}<input type="hidden" name="parcelas_personalizadas[${i}][numero]" value="${i+1}"></td>
...
function recalcularPersonalizado(){
    ...
    let vals = inputs.toArray().map(el => parseFloat($(el).val().replace(/\./g,'').replace(',','.'))||0);
    let soma = vals.reduce((a,b) => a+b, 0);
    ...
    if(document.activeElement.classList.contains('valor-parcela-personalizada')){
        ...
        let rest = inputs.length - idx - 1;
        let saldo = total - vals.slice(0,idx+1).reduce((a,b) => a+b, 0);
        let base = Math.floor((saldo/rest)*100)/100;
        let rem = Math.round((saldo - base*rest)*100)/100;
...
$('#adicionar-item').click(function(){
    let i = $('#tabela-itens tr').length;
    let o = '';
    @foreach($produtos as $p)
    o += `<option value="{{ $p->id }}">{{ $p->nome }}</option>`;
    @endforeach
...
$('#form-venda').submit(function(e){
    if($('#forma_pagamento').val()==='Personalizado'){
        let total = parseFloat($('#total-venda').val().replace(/\./g,'').replace(',','.'))||0;
        let soma = $('.valor-parcela-personalizada')
            .toArray()
            .reduce((s, el) => s + (parseFloat($(el).val().replace(/\./g,'').replace(',','.'))||0), 0);
